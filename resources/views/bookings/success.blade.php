...
@php
    $downPayment = (int) ceil($booking->total_price * 0.1);
    $totalTransfer = $downPayment + (int) ($booking->unique_code ?? 0);
@endphp
            ...
            <div class="scroll-reveal scroll-reveal--fade-in-up bg-gray-50 rounded-xl p-6 mb-8 text-left">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between">
                        <span class="text-gray-500">DP 10%</span>
                        <span class="font-semibold text-emerald-700">Rp {{ number_format($downPayment, 0, ',', '.') }}</span>
                    </div>
                    ...
                    <div class="flex justify-between pt-2 border-t border-dashed border-gray-200">
                        ...
                        <span class="font-bold text-emerald-700">Rp {{ number_format($totalTransfer, 0, ',', '.') }}</span>
                    </div>
                    <div class="flex justify-between">
                        <span class="text-gray-500">Sisa Pelunasan</span>
                        <span class="font-medium text-gray-500">Rp {{ number_format($booking->total_price - $downPayment, 0, ',', '.') }}</span>
                    </div>
            ...
            <div class="scroll-reveal scroll-reveal--fade-in-up mb-8 text-left">
                <div class="bg-gradient-to-br from-emerald-50 via-white to-amber-50 border-2 border-emerald-100 rounded-2xl p-6 lg:p-8 shadow-sm">
                    ...
                    <div class="bg-gradient-to-br from-emerald-600 to-teal-600 rounded-xl p-5 mb-6 text-center shadow-md">
                        <p class="text-emerald-100 text-xs mb-1">Pembayaran DP 10%</p>
                        <p class="text-2xl lg:text-3xl font-bold text-white font-['Playfair_Display']">Rp {{ number_format($totalTransfer, 0, ',', '.') }}</p>
                        <p class="text-emerald-200 text-xs mt-1.5">DP 10% dari Rp {{ number_format($booking->total_price, 0, ',', '.') }}@if($booking->unique_code) + kode unik {{ $booking->unique_code }}@endif</p>
                        <p class="text-emerald-300/70 text-[10px] mt-1">Sisa pelunasan Rp {{ number_format($booking->total_price - $downPayment, 0, ',', '.') }} dibayar saat check-in</p>
                    </div>
                    ...
                    <div class="space-y-4 mb-6">
                        ...
                        <div class="flex gap-4">
                            ...
                            <div class="pb-4 flex-1">
                                <h4 class="font-semibold text-gray-900 text-sm">Transfer DP 10% ke Rekening BCA</h4>
                                <p class="text-xs text-gray-500 mt-1">Lakukan transfer <strong>DP 10% (Rp {{ number_format($totalTransfer, 0, ',', '.') }})</strong> ke rekening berikut:</p>
                        ...
                        <div class="flex gap-4">
                            ...
                            <div class="flex-1">
                                ...
                                <a href="https://wa.me/6285136907907?text=Halo%20Embun%20Village%2C%20saya%20telah%20melakukan%20pembayaran%20DP%20untuk%20booking%20%23{{ $booking->id }}%20-%20{{ urlencode($booking->room->name) }}%0A%0ANama%3A%20{{ urlencode($booking->guest_name) }}%0ADP%2010%25%3A%20Rp{{ number_format($totalTransfer, 0, '', '') }}%0A(Sisa%20pelunasan%3A%20Rp{{ number_format($booking->total_price - $downPayment, 0, '', '') }})%0A%0ABerikut%20bukti%20transfernya%3A" target="_blank" class="mt-3 w-full inline-flex items-center justify-center gap-2 px-5 py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-semibold text-sm transition-all shadow-md hover:shadow-lg active:scale-[0.98]">
